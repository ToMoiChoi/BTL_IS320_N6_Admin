              ...
              <div className="bg-white rounded-2xl p-6 shadow-sm">
                  ...
                  <div className="mb-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {storageOptions.map((storage, idx) => {
                        // Check availability for current color
                        const isAvailableForColor = allSpecs.some(s => s.BoNho === storage && s.MauSac === selectedSpec?.MauSac);
                        
                        return (
                        <button
                          key={idx}
                          onClick={() => {
                            let spec = allSpecs.find(s => s.BoNho === storage && s.MauSac === selectedSpec?.MauSac);
                            if (!spec) spec = allSpecs.find(s => s.BoNho === storage);
                            if (spec) setSelectedSpec(spec);
                          }}
                          className={`px-4 py-2 rounded-lg border-2 text-sm font-medium transition relative ${
                            selectedSpec?.BoNho === storage
                              ? 'border-red-500 bg-red-50 text-red-600'
                              : 'border-gray-200 hover:border-gray-400 text-gray-700'
                          } ${!isAvailableForColor ? 'opacity-80' : ''}`}
                        >
                          {storage}
                        </button>
                      )})}
                  ...
                  <div className="mb-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {colorOptions.map((option, idx) => {
                         // Calculate price for this color with CURRENT storage
                         const specForCurrentStorage = allSpecs.find(s => s.MauSac === option.color && s.BoNho === selectedSpec?.BoNho);
                         const displayPrice = specForCurrentStorage ? specForCurrentStorage.GiaBan : option.price;
                         const isMatchStorage = !!specForCurrentStorage;

                         return (
                        <button
                          key={idx}
                          onClick={() => {
                             let spec = allSpecs.find(s => s.MauSac === option.color && s.BoNho === selectedSpec?.BoNho);
                             if (!spec) spec = option.spec; // Fallback to the default spec for this color
                             if (spec) setSelectedSpec(spec);
                          }}
                          className={`px-4 py-2 rounded-lg border-2 text-sm transition text-left min-w-[120px] ${
                            selectedSpec?.MauSac === option.color
                              ? 'border-red-500 bg-red-50'
                              : 'border-gray-200 hover:border-gray-400'
                          }`}
                        >
                          <div className="font-medium text-gray-800">{option.color}</div>
                ...
                <div className="mb-6">
                    <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-sm font-medium ${
                        (selectedSpec?.SoLuong || 0) > 0 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                    }`}>
                        <span className="text-lg">{(selectedSpec?.SoLuong || 0) > 0 ? '✓' : '✕'}</span>
                        {(selectedSpec?.SoLuong || 0) > 0 ? `Còn hàng (${selectedSpec.SoLuong})` : 'Hết hàng'}
                    </div>
                </div>
              </div>
          ...
          <div className="mt-8 bg-white rounded-2xl p-6 shadow-sm">
            ...
            <div className="overflow-hidden rounded-xl border border-gray-200">
              <table className="w-full">
                <tbody>
                  {[
                    { label: "Màn hình", value: `${product?.PhienBan || ""} ${product?.KichThuoc ? `(${product.KichThuoc})` : ""}`.trim() || "Đang cập nhật" },
                    { label: "Hệ điều hành", value: product?.HeDieuHanh || "Đang cập nhật" },
                    { label: "Chip xử lý", value: product?.Chitset || "Đang cập nhật" },
                    { label: "RAM", value: product?.RAM || "Đang cập nhật" },
                    { label: "Bộ nhớ trong", value: selectedSpec?.BoNho || "Đang cập nhật" },
                    { label: "Camera sau", value: product?.Camera || "Đang cập nhật" },
                    { label: "Pin", value: product?.Pin === "string" ? "Đang cập nhật" : product?.Pin || "Đang cập nhật" },
                    { label: "Thẻ SIM", value: product?.TheSim || "Đang cập nhật" },
