      ...
      const checkPayment = async () => {
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (data.paid) {
              setPaymentStatus("✅ Thanh toán thành công! Đơn hàng đã được xác nhận và đang xử lý...");
              // Auto confirm payment
              setTimeout(async () => {
                await clearCart(token);
                setShowPaymentModal(false);
                alert("🎉 Đơn hàng của bạn đã được xác nhận thành công!\n\nMã đơn hàng: #" + currentOrder.MaDH + "\nTrạng thái: Đang xử lý\n\nCảm ơn bạn đã mua hàng!");
                history.push(`/bill/${currentOrder.MaDH}`);
              }, 2000);
              clearInterval(intervalId);
            } else {
              setPaymentStatus("🔍 Chưa nhận được thanh toán - đang kiểm tra...");
            }
          }
        } catch (error) {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 px-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 relative">
            ...
            <div className="text-center">
              ...
              {/* Payment Status */}
              {paymentStatus && (
                <div className={`p-4 rounded-lg mb-4 text-sm font-semibold border-2 ${
                    paymentStatus.includes("✅") || paymentStatus.includes("thành công")
              ...
              <button
                onClick={handlePaymentCancel}
                className="w-full border border-gray-300 text-gray-700 font-bold py-3 rounded-lg hover:bg-gray-50 transition"
              >
                Hủy đơn hàng
              </button>
